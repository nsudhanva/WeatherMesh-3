...
def validate(fields, real=None, era_mesh=None):
    ...
    if real is not None and era_mesh is not None:
        ...
        if finite.size:
            precip_capped_frac = float((finite > outputs.PRECIP_CAP_MM).mean())
            precip_p99_mm = float(np.percentile(np.clip(finite, 0, outputs.PRECIP_CAP_MM), 99))
    ...
    m["precip_reliable"] = bool(precip_raw_finite and precip_capped_frac < PRECIP_CAPPED_FRAC_MAX)
    return m
